...
def getBoard():
    htmlCode = requests.get('http://www.hacker.org/coil/', params={'name': 'SpaceMonkey', 'password': 'schniffi123'}).content
...
def sendSolution(sol, x, y):
    ...
    params = urllib.urlencode({'name': 'SpaceMonkey', 'password': 'schniffi123', 'x': str(y), 'y': str(x), 'path': sol_string})
